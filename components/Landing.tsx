...
export default function Landing({locale}:{locale:string}){
 ...
 return <>
  ...
  <section className="hero"><div className="badge">{t.heroBadge}</div><h1><span className="blue">{t.title1}</span><br/>{t.title2}</h1><p className="lead">{t.lead}</p><div className="actions"><a className="btn btnMain" href={`/${locale}/dashboard`}>{t.connect}</a><a className="btn" href="https://t.me/ceo_plankton" target="_blank">{t.openBot}</a></div></section></div>
